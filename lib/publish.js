function publish (options) {
    ...
    request[method](url)
      ...
      .end((err, res) => {
        if (!err && res.ok && res.body.status === 201 && res.body.id !== null) {
          ...
          pushBook(bookizArc, bookrc, token)
        } else {
          let errorMessage
  ...
  function pushBook (bookizArc, bookrc, token = bookizArc.token) {
    const book = {}

    book.title = bookrc.name || 'Untitled'
    book.has_page_numbers = bookrc.has_page_numbers || 'false'
    book.status = bookrc.status || ''   /* Required for automated publishing only. */
   ...
   */ 
    ...
    function printComplete (response) {
      if (requestStatus.every(elem => elem === 1) && requestStatus.length === float2int(length / PAGES_CHUNK_SIZE) + 1) {
        ...
        console.log(chalk.bold.cyan(' [100%] Manuscript patch completed successfully.'))
        console.log(chalk.bold.yellow(' Book was patched into your Bubblin Account. Check online @ https://bubblin.io!'))
      }
    }
  }
}
